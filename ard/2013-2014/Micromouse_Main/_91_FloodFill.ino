#include "_95_Cell.h"
#include "_94_CellStack.h"
#include "_92_FloodFill.h"
...
int movementsForward = 0;
bool bufferMoveDone = 0;
...
void flood(int x, int y) {
    ...
    if (!inGoal()) {
        ...
        if (m_cells[x][y].m_distance != _min(northDistance, eastDistance, southDistance, westDistance) + 1) {

            // Set the value to the min plus one
            m_cells[x][y].m_distance = _min(northDistance, eastDistance, southDistance, westDistance) + 1;
...
void moveTowardsGoal(void) {
    ...
    //moveForward(); // Move the robot forward3
    moveForward(1); //Don't Move
}

void moveForward(bool bufferMove) {
    ...
    //int oldm_x = m_x;
      ...
      switch (m_d) {
          case NORTH:
              ...
              //  m_cells[m_x][oldm_y + i].m_explored = true;
              break;
          case EAST:
              m_x += 1;
              //for(int i = 1; i <= timesForward; i++)
              //  m_cells[oldm_x + i][m_y].m_explored = true;
              break;
          case SOUTH:
              m_y -= 1;
              //for(int i = 1; i <= timesForward; i++)
          ...
          case WEST:
              m_x -= 1;
              //for(int i = 1; i <= timesForward; i++)
              //  m_cells[oldm_x - i][m_y + i].m_explored = true;
              break;
      }
      Serial.print(m_x);
      Serial.print("    ,    ");
...
int _min(int one, int two, int three, int four) {
