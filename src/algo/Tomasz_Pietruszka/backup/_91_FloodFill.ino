...
void FloodFill::simpleSolve() {
    ...
    while (true) {
        ...
        int s_steps = m_steps;
        if (ALGO_COMPARE) {
            //// // "Simple explore in " // m_steps // " steps" // std::endl;
        }
        ...
        if (ALGO_COMPARE) {
            //// // "Simple solve in " // (m_steps - s_steps)/2 // " steps" // std::endl;
            return;
        }
    }
}
...
void FloodFill::extensiveSolve() {
    ...
    while (true){
        ...
        int s_steps = m_steps;
        if (ALGO_COMPARE) {
            //// // "Extensive explore in " // m_steps // " steps" // std::endl;
        }
        ...
        if (ALGO_COMPARE) {
            //// // "Extensive solve in " // (m_steps - s_steps)/2 // " steps" // std::endl;
            return;
        }
    }
}
...
void FloodFill::walls(){
    ...
    m_cells[m_x][m_y].setWall(m_d, _wallFront());
    m_cells[m_x][m_y].setWall((m_d+1)%4, _wallRight());
    m_cells[m_x][m_y].setWall((m_d+3)%4, _wallLeft());
...
void FloodFill::flood(int x, int y){
    ...
    if (!inGoal(x, y)){
        ...
        if (m_cells[x][y].getDistance() != min4(northDistance, eastDistance, southDistance, westDistance) + 1){

            // Set the value to the min plus one
            m_cells[x][y].setDistance(min4(northDistance, eastDistance, southDistance, westDistance) + 1);
...
int FloodFill::min4(int one, int two, int three, int four){
    int firstMin = one;
    int secondMin = three;
    if (two <= one){
        firstMin = two;
...
//Eriralprintln("Proceed to Path exited");
        ...
        if (!m_centerReached && inGoal(m_x, m_y)) {
            ...
            if (ALGO_COMPARE) {
                // // "Extensive center in " // m_steps // " steps" // std::endl;
            }
        }
    }
...
void FloodFill::moveOneCell(Cell* target){
    ...
    if (isOneCellAway(target)){
        ...
        // // "ERROR: Tried to move to cell (" // target->getX() // ","
...
void FloodFill::doUpdatesForCurrentCell(std::stack<Cell*>* unexplored) {
    ...
    if (!_wallLeft() && getLeftCell()->getPrev() == NULL){
    ...
    if (!_wallRight() && getRightCell()->getPrev() == NULL){
    ...
    if (!_wallFront() && getFrontCell()->getPrev() == NULL){
...
void FloodFill::bffExplore(std::stack<Cell*>* path){
    ...
    while (true) {
        ...
        if (ALGO_COMPARE) {
            // // "Simple center in " // m_steps // " steps" // std::endl;
        }
...
void FloodFill::dobffCellUpdates() {
    ...
    if (!_wallLeft() && getLeftCell()->getPrev() == NULL && getLeftCell() != &m_cells[0][0]){
        getLeftCell()->setPrev(&m_cells[m_x][m_y]);
    }
    if (!_wallFront() && getFrontCell()->getPrev() == NULL && getFrontCell() != &m_cells[0][0]){
        getFrontCell()->setPrev(&m_cells[m_x][m_y]);
    }
    if (!_wallRight() && getRightCell()->getPrev() == NULL && getRightCell() != &m_cells[0][0]){
        getRightCell()->setPrev(&m_cells[m_x][m_y]);
    }
}
...
bool FloodFill::proceedToCheckpoint(std::stack<Cell*> path) {
    ...
    while (!path.empty()) {
    //Eriralprintln("Proceedting to Checkpoint Not Empty");
