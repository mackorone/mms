...
void History::resetModifiedCells() {
    ...
    while (!m_modifiedCells.empty()) {
        ...
        //Eriralprintln("While- looping");
        std::list<Cellmod> cellList = m_modifiedCells.front();
        m_modifiedCells.pop_front();

        for (std::list<Cellmod>::iterator it = cellList.begin(); it != cellList.end() ; ++it) {
             //Eriralprintln("For-looping");
    ...
    m_size = 0;
    while (!m_stacks.empty()) {
      //Eriralprintln("Looping in stack emptying");
        m_stacks.pop();
    }
    while (!m_stackReferenceCounts.empty()) {
       //Eriralprintln("Looping in stack-referenceCounts");
        m_stackReferenceCounts.pop_front();
    }
    /*
    while (!m_modifiedCells.empty()) {
       //Eriralprintln("Looping in 2");
        m_modifiedCells.pop_front();
    }
    */
    //Eriralprintln("Doing Last two calls of reset Cells");
    m_stacks.push(m_checkpointStack);
    m_stackReferenceCounts.push_back(1);
    //Eriralprintln("right before reset exits");
}
