...
void Header::draw() {
    ...
    // This cute little block of code initializes the column starting positions
    // once the mouse algorithm has been initialized, making things align better
    static bool alreadyExecuted = false;
    if (!alreadyExecuted && m_mouseAlgorithm != nullptr) {
        m_columnStartingPositions = getColumnStartingPositions();
