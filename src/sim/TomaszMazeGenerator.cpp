...
std::vector<std::vector<BasicTile>> TomaszMazeGenerator::convertToBasicMaze() {
    std::vector<std::vector<BasicTile>> returnMaze;
    for (int x = 0; x < m_width; x += 1) {
        ...
        for (int y = 0; y < m_height; y += 1) {
            ...
            tile.x = x;
            tile.y = y;
            tile.walls = getTile(x, y)->walls;
            column.push_back(tile);
        }
        returnMaze.push_back(column);
    }
    return returnMaze;
}

void TomaszMazeGenerator::generateMaze() {

    // Declare width and height locally to reduce function calls
    m_width = P()->generatedMazeWidth();
    m_height = P()->generatedMazeHeight();
    ...
    const float deadEndBreak = P()->tomDeadEndBreakChance();
    ...
    while (m_stack.empty() == false) {
        ...
        if (possible == 0) {
            ...
            const static float straightFactor = P()->tomStraightConst();
...
Direction TomaszMazeGenerator::getDirectionToMove(float moveConst, std::map<Direction, bool> choices){
    ...
    if (m_direction != UNDEFINED && choices[m_direction] != false && possible != 1) {
        if (SimUtilities::getRandom() <= moveConst) {  // if random variable is below threshold move in last direction
            directionToMove = m_direction;
        } else {
            auto item = choices.begin();
            std::advance( item, std::floor(SimUtilities::getRandom() * 4 ));
...
void TomaszMazeGenerator::breakGradientWall(int xPos, int yPos) {
    ...
    const static float breakThreshold = P()->tomDeadEndBreakThreshold();
