...
Maze::Maze() {
    ...
    if (!P()->useMazeFile() && P()->saveGeneratedMaze()) {
        std::string mazeFilePath = Directory::getResMazeDirectory() + P()->generatedMazeFile();
        MazeFileUtilities::saveMaze(basicMaze, mazeFilePath);
        L()->info(
            "Saving maze generated by \"%v\" maze algorithm to \"%v\"",
            P()->mazeAlgorithm(), mazeFilePath);
    }
    ...
    for (int i = 0; i < P()->mazeRotations(); i += 1) {
        ...
        L()->info("Rotating the maze counter-clockwise (%v).", i);
    }
